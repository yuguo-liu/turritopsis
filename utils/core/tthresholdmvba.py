...
def tthresholdmvba(sid, pid, N, f, C, g, PKs, SK, input, decide, receive, send, ty='s', predicate=lambda x: True, logger=None):
    ...
    for j in range(N):
        ...
        rbc = gevent.spawn(reliablebroadcast, sid + 'WRBC' + str(j), pid, N, f, j,
                           rbc_input, rbc_recvs[j].get, make_rbc_send(j), predicate)
