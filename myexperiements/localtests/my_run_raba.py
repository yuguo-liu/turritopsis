...
def simple_coin_router(N, maxdelay=0.1, seed=None):
    """Builds a set of connected channels, with random delay
     @return (receives, sends)
     """
    rnd = random.Random(seed)
...
def _test_twovalueagreement(N=4, f=1, seed=None):
    ...
    for i in range(N):
        ...
        t = gevent.spawn(pisaaragreement, sid, i, N, f, coins[i],
    ...
    x = 0
    # print(x)
    l_i = [x + random.randint(0, 1) for _ in range(N)]
    # l_i = [0, 0, 1, 1]
    for i in range(N):
        inputs[i].put_nowait(l_i[i])
