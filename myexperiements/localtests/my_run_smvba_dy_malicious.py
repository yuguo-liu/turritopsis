...
def _test_smbva(N=16, f=5, leader=None, seed=None):
    ...
    # sample malicious node
    malicious_nodes = []
    for _ in range(f):
        mn = random.randint(0, N - 1)
        while mn in malicious_nodes:
            mn = random.randint(0, N - 1)
        malicious_nodes.append(mn)
    
    malicious_nodes = sorted(malicious_nodes)
    print(f"\033[30m[INFO]\033[0m Malicious nodes: {malicious_nodes}")
    ...
    for i in range(N):
        ...
        inputs_content.append(node_input)
        print(f"\033[30m[INFO]\033[0m Input to node {i} has been provided as: {node_input}")

    for i in range(N):
        mali = 1 if i in malicious_nodes else 0
        t = Greenlet(speedmvba, sid, i, N, f, 0, C, PK2s, SK2s[i], ePKs, eSKs[i], group.hash(123, G), 's',
                     inputs[i].get, outputs[i].put_nowait, recvs[i], sends[i], p, malicious=mali) 
    ...
    try:
        ...
        try:
            ...
            assert outs[0] in inputs_content, "\033[31m[ERROR]\033[0m Agreed content is not from the node's input"
            print("\033[32m[PASS]\033[0m Agreed content is from the node's input")
