...
from charm.core.math.elliptic_curve import ec_element
...
def polynomials_over():
    ...
    class Polynomial(object):
        def __init__(self, coeffs):
            ...
            for i in range(len(self.coeffs)):
                if type(self.coeffs[i]) is int:
                    self.coeffs[i] = group.init(ZR, self.coeffs[i])
        ...
        @classmethod
        def interpolate_at(cls, shares, x_recomb=group.init(ZR, 0)):
            # shares are in the form (x, y=f(x))
            if type(x_recomb) is int:
                x_recomb = group.init(ZR, x_recomb)
        ...
        def interpolate(cls, shares):
            x = cls([group.init(ZR, 0), group.init(ZR, 1)])  # This is the polynomial f(x) = x
            one = cls([group.init(ZR, 1)])  # This is the polynomial f(x) = 1
            xs, ys = zip(*shares)
            ...
            def lagrange(xi):
                ...
                den = reduce(mul, [xi - xj for xj in xs if xj != xi], group.init(ZR, 1))
                #p = num * cls([1 / den])
                p = num / den
        ...
        def interpolate_fft(cls, ys, omega):
            ...
            coeffs = [b / n for b in fft_helper(ys, 1 / omega, group)]
            return cls(coeffs)
        ...
        def random(cls, degree, y0=None):
            coeffs = [group.random() for _ in range(degree + 1)]
            if y0 is not None:
                if type(y0) is int:
                    y0 = group.init(ZR, y0)
        ...
        def __divmod__(self, divisor):
            ...
            while remainder.degree() >= divisor_deg:
                ...
                monomial_zeros = [group.init(ZR, 0) for _ in range(monomial_exponent)]
...
def fft(poly, omega, n):
    ...
    return fft_helper(padded_coeffs, omega)
