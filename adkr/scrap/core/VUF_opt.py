...
def dri_my_key(pk, ski, id):
    rnd = random.Random()
    alpha = (int(rnd.random()*100) % n) + 1
    beta = (int(rnd.random()*100) % n) + 1
    alpha = group.init(ZR, alpha)
    beta = group.init(ZR, beta)
    A, u2 = pk
    p1 = g1 ** alpha
    p2 = g1 ** beta
    p1_h = (h1 ** (-alpha)) * (h2 ** (-beta))
    p2_h = (h3 ** (-alpha)) * ((h4 ** (-beta))) * ski
    return (A, u2, p1, p2, p1_h, p2_h), (ski, alpha, beta)
...
def vuf_combine(sigmaset):
    ...
    for i in range(len(s)):
        for j in range(6):
            out[j] *= (sigmaset[i][j] ** lagrange_x(s, sx[i], 0))
    return out
