...
def _test_spbc(N=7, f=2, leader=None, seed=None):
    ...
    for i in range(N):
        ...
        t = Greenlet(strongprovablebroadcast, sid, i, N, f, 0, C, PK2s, SK2s[i], leader, input, output_list.put_nowait, recvs[i], sends[i], 0, malicious=1 if i in malicious_nodes else 0)
    ...
    print(f"\033[33m[TIME]\033[0m broadcast time: {(time.time()-s_t) * 1000} ms")
