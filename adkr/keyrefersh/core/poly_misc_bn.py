...
def lagrange_c1(S, j):
    ...
    assert j in S
    num = reduce(mul, [0 - jj - 1 for jj in S if jj != j], ONE1)
    den = reduce(mul, [j - jj for jj in S if jj != j], ONE1)  # noqa: E272
    # assert num % den == 0
    return num / den
...
def lagrange(S, j):
    ...
    assert j in S
    num = reduce(mul, [0 - jj     for jj in S if jj != j], ONE1)
    den = reduce(mul, [j - jj     for jj in S if jj != j], ONE1)  # noqa: E272
    # assert num % den == 0
    return num / den
...
def lagrange_at_x(S, j, x):
    ...
    assert j in S
    num = reduce(mul, [x - jj     for jj in S if jj != j], ONE1)
    den = reduce(mul, [j - jj     for jj in S if jj != j], ONE1)  # noqa: E272
    # assert num % den == 0
    return num / den
