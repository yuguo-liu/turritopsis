...
def _test_rbc1(N=4, f=1, leader=None, seed=None):
    ...
    for i in range(N):
        ...
        t = gevent.spawn(ADKR_old_c, sid, i, C_o, C_n, f, 2, g1, thpk, thpks, thsks[i], ePKs1, eSKs1[i], sends[i], sends2[i], recvs[i])
        # t.start()
        threads.append(t)
    threads2 = []
    for i in range(N):
        t2 = gevent.spawn(adkr_new, i, sid, C_n, N, N, f, f, 2, g1, ePKs1, eSKs1[i], sends2[i], recvs2[i])
        # t2.start()
        threads2.append(t2)
