...
def recv_loop(pid, recv_func, recv_queues, C):
    while True:
...
def speedmvba(sid, pid, N, f, l, C, PK2s, SK2, input, decide, receive, send, predicate, logger=None):
    ...
    recv_loop_thred = Greenlet(recv_loop, pid, receive, recv_queues, C)
    ...
    def views():
        ...
        while not start_wait_for_halt.is_set():
            ...
            for j in range(N):
                ...
                spbc_input = my_spbc_input.get if C[j] == pid else None
                spbc = gevent.spawn(strongprovablebroadcast, sid + 'SPBC' + str(C[j]), pid, N, f, l, C, PK2s, SK2, C[j],
                                    spbc_input, spbc_s1_list[j].put_nowait, spbc_recvs[r][j].get, make_spbc_send(C[j], r),
                        ...
                        if aba_tag == 'vote':
                            ...
                            if vote_msg[1] == 1:
                                ...
                                if len(vote_yes_shares) == N - f - l:
                                    ...
                                    halt_msg = (Leader, 2, vote_msg[2], tuple(list(vote_yes_shares.items())[:N - f - l]))
                                    # broadcast(('MVBA_HALT', r, pid, ("halt", halt)))
