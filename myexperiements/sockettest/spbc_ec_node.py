...
from speedmvba_dy.core.spbc_high import SPBChigh
...
def load_key(id, N):
    ...
    ePKs = []
    for i in range(N):
        with open(os.getcwd() + '/keys-' + str(N) + '/' + 'ePK1-' + str(i) + '.key', 'rb') as fp:
    ...
    return sPK2s, sSK2, thsk, thpks, thpk

class SPBCECNode(SPBChigh):
    ...
    def __init__(self, sid, id, f, N, K, bft_from_server: Callable, bft_to_client: Callable,
        ...
        self.sPKs, self.sSK, self.thsk, self.thpks, self.thpk = load_key(id, N)
        ...
        SPBChigh.__init__(self, sid, id, N, f, K, self.g1, self.g2, self.sPKs, self.sSK, self.thpk, self.thpks, self.thsk,
