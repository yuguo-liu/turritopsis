...
class Adkrhbacss():
    ...
    def run_bft(self):
        ...
        for r in range(round):
            ...
            if pid in self.C_o:
                if self.debug:
                    gevent.spawn(shared_coin, self.sid + 'ADKR_high', pid, self.N, self.f, 0, self.C_n, self.g1,
                                 self.thpks, self.thsk,
                                 adkr_old_recv[r].get, adkr_new_send)
                else:
                    gevent.spawn(shared_coin, self.sid + 'ADKR_high', pid, self.N, self.f, 0, self.C_n, self.g1,
                                 self.thpks, self.thsk,
                                 adkr_old_recv[r].get, adkr_new_send)
            ...
            if pid in self.C_n:
                if self.debug:
                    t = gevent.spawn(ADKR_old_c, self.sid + 'ADKR_high', pid, self.C_o, self.C_n, self.f, self.K, self.g1, self.h,
                                 self.thpk, self.thpks, self.thsk, self.ePKS, self.eSK,
                                 adkr_new_send, adkr_old_send, adkr_new_recv[r].get, self.logger)
                else:
                    t = gevent.spawn(ADKR_old_c, self.sid + 'ADKR_high', pid, self.C_o, self.C_n, self.f, self.K, self.g1, self.h,
                                 self.thpk, self.thpks, self.thsk, self.ePKS, self.eSK,
                                 adkr_new_send, adkr_old_send, adkr_new_recv[r].get, self.logger)
        ...
        self.send(0, (-1, ('ADKR_COUNT', ((e_time - self.count_time)/(round-1), diff))))
        tt = time.time()
        if self.id == 0:
            co = 0
            co_n = 0
