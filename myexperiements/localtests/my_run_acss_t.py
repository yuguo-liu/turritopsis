...
import phe
from adkr.adkr_high.adkg_adp.acss_rbc import completesecretsharing
...
def _test_rbc1(N=4, f=1, dealer=None, seed=None):
    ...
    for i in range(N):
        ...
        t = gevent.spawn(completesecretsharing, sid, i, N, f, C_o, N, f, C_n, g1, dealer, ePKs1, eSKs1[i], thpk, thsks[i],  input, output[i].put_nowait, recvs[i], sends[i], None)
        # t = gevent.spawn(work)
        threads.append(t)
    m = 100
    leader_input.put(m)
