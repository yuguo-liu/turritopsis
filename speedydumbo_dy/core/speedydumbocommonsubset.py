...
def speedydumbocommonsubset(pid, N, f, l, C, r, reconfig, last_pb_value, pb_values_out, last_pb_values_out, pb_proof_out, vacs_in, vacs_out, logger=None):
    ...
    def wait_for_pb_value(leader):
        ...
        # if pid == 4:
        #     print("node %d get PB value in ACS from leader %d" % (pid, C[leader]))
        ...
        for j in range(N):
            if pb_proofs_vector[j] is not None:
                ...
                try:
                    ...
                    count1 += 1
                    # logger.info('node %d collects one more value in ACS 1 from leader %d' % (pid, C[j]))
                except AssertionError as e:
                    # print("node %d finds None PB value in ACS from leader %d" % (pid, j))
                    # logger.info('node %d finds None PB value in ACS from leader %d' % (pid, C[j]))
                    pb_value_threads[j].join()
                    assert pb_values[j] is not None
                    # count += 1
                    # logger.info('node %d collects one more value in ACS 2 from leader %d' % (pid, C[j]))
                ...
                try:
                    assert pb_values[j] is not None  # TODO: Check delivered value consistent to pb proof
                    pb_left[C[j]] = pb_values[j]
                    # print("node %d collects one more value in ACS 1 from leader %d" % (pid, j))
                except AssertionError as e:
                    # pb_left[C[j]] = None
                    # print("node %d finds None PB value in ACS from leader %d" % (pid, j))
                    # logger.info('node %d finds None PB value in ACS from leader %d' % (pid, j))
                    pass

                pb_value_threads[j].kill()
                pb_values[j] = None
        if r % reconfig != 1 or r == 1:
            # logger.info('pb vector:%s' % pb_proofs_vector)
            for j in range(N, N*2):
                # print(pid, "pb values", last_pb_value.keys())

                if pb_proofs_vector[j] is not None:
                    # logger.info('%d in last round is not None in acs output' % C[j-N])
                    if C[j-N] in last_pb_value.keys():
                        # print("add last round's", C[j - N])
                        # logger.info('add last round %d' % C[j - N])
                        pb_values[j] = last_pb_value[C[j-N]]
                        count2 += 1
                    else:
                        # one missing pb, need to be reload
                        try:
                            # logger.info('pb_values out %d' % C[j - N])
                            pb_values[j] = last_pb_values_out[j-N]()
                            # print("get from last round")
                            count2 += 1
                        except Exception as e:
                            logger.info('missing one!')
                            print("missing one")
                            count += 1
                            continue
    ...
    pl = pb_left.keys()
    # logger.info('%d, count is %d, pb_left %s' % (r, count, pl))
