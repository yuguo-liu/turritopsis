from pypairing import PyFq, PyFq2, GT as PyFq12, PyFqRepr, G1 as PyG1, G2 as PyG2, ZR as PyFr, hashg1s, hashfrs, dotprod, blsmultiexp, condense_list
import random
import re
import struct
import gc
...
def mulexp(zrlista, zrlistb):
    #pyfrlista, pyfrlistb = [], []
    assert len(zrlista) == len(zrlistb)
    pyfrlista = [item.pyg1 for item in zrlista]
    pyfrlistb = [item.val for item in zrlistb]
    #for i in range(len(zrlista)):
    #    pyfrlista.append(zrlista[i].val)
    #    pyfrlistb.append(zrlistb[i].val)
    out = G1(blsmultiexp(pyfrlista, pyfrlistb))
    return(out)